...
router.post('/signup', async function (req, res, next) {
  ...
  try {
    const userInDB_email = await User.findOne({ email: email });
    if (userInDB_email) {
      res.render('auth/signup', { error: `There already is a user with email ${email}` });
      return;
    }
      const userInDB_username = await User.findOne({ username: username });
    if (userInDB_username) {
      res.render('auth/signup', { error: `There already is a user with username ${email}` });
      return;
    } else {
...
router.get('/logout', (req, res, next) => {
  req.session.destroy((err) => {
    if (err) {
      ...
      res.clearCookie('basic-auth-app')
